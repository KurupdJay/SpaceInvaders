...
# healing
HEALING_CROSS = pygame.image.load(os.path.join("healingcross.png"))
...
class Heal:
    def __init__(self, x, y):
        self.x = x
        self.y = y
        self.heal_img = pygame.transform.scale(HEALING_CROSS, (40, 40))
...
def main():
    ...
    heal = Heal(random.randrange(20, WIDTH-20), random.randrange((int(HEIGHT / 2 - 200)), (HEIGHT+20)))
        ...
        if len(enemies) == 0:
            ...
            for i in range(wave_length):
                if level == 1 or 2 or 3 or 4:
                    enemy = Enemy(random.randrange(50, WIDTH - 100), random.randrange(-2000, -100),
                               random.choice(["red", "blue", "green"]))
                    enemies.append(enemy)
                else:
                    enemy = Enemy(random.randrange(50, WIDTH - 100), random.randrange(-2000 * (level/4), -100),
                              random.choice(["red", "blue", "green"]))
                    enemies.append(enemy)
            for i in range(healing_pad):
                if level >= 2:
                    heal = Heal(random.randrange(0, WIDTH - 20), random.randrange(int(HEIGHT / 2 - 200), (HEIGHT + 20)))
                    healing.append(heal)
